...
How **uaslog** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `uaslog` lives in the **Drone / counter-UAS** cluster.
...
| from | relation | to |
|---|---|---|
| `uaslog` | JSON findings compose with | [`awesome-drone-warfare-osint`](https://github.com/cognis-digital/awesome-drone-warfare-osint), [`frontline-drones`](https://github.com/cognis-digital/frontline-drones) |
| `uaslog` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `uaslog` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `uaslog` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the drone / counter-uas cluster.** `uaslog` output feeds the next tool:
```bash
uaslog ... --format json > out.json          # this tool's findings (see `uaslog --help`)
...
uaslog ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
uaslog ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
uaslog ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `uaslog`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `uaslog` slots
