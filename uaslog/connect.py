"""Native cognis-connect emit for uaslog — forward findings to any platform.

Maps uaslog's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    uaslog ... --format json | uaslog-emit --to stix
    uaslog-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('event', '').split(' ')[0]
